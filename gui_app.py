...
TEXT = {
    "zh": {
        ...
        "segment_workers_help": "同时处理多少份试卷。数值越大越快，但更容易遇到 API 限流或网络错误。建议先用 4。",
        "score_workers": "评分并发数 score workers",
        "score_workers_help": "同时评分多少道题。建议先用 8，稳定后再提高。",
        "segment_input": "切割输入方式 segment input",
        "segment_input_help": "rough 表示先本地粗切再交给 AI，省 token；full 表示整份试卷交给 AI，可能更稳但更贵。",
        ...
        "thinking": "思考模式 / 推理强度",
        "thinking_tip": "建议：segment 和 score 默认关闭 thinking；review-select 默认开启 thinking medium。",
        "segment_thinking": "切割思考模式 segment thinking",
        "segment_thinking_help": "控制切割任务是否启用模型思考。切割通常是结构化抽取，默认 omit 最省钱。",
        "score_thinking": "评分思考模式 score thinking",
        "score_thinking_help": "控制评分任务是否启用模型思考。默认 omit，适合大批量低成本评分。",
    ...
    "en": {
        ...
        "segment_workers_help": "How many papers to segment concurrently. Larger is faster but may hit API/network limits. Start with 4.",
        "score_workers": "Score workers",
        "score_workers_help": "How many items to score concurrently. Start with 8, then increase if stable.",
        "segment_input": "Segment input",
        "segment_input_help": "rough first makes local rough chunks before AI to save tokens; full sends the whole paper to AI, which may be more robust but more expensive.",
        ...
        "thinking_tip": "Recommendation: segment and score keep thinking off; review-select uses thinking medium by default.",
        "segment_thinking": "Segment thinking",
        "segment_thinking_help": "Whether to enable model thinking for segmentation. Segmentation is mostly extraction; omit is cheaper.",
        "score_thinking": "Score thinking",
        "score_thinking_help": "Whether to enable model thinking for scoring. Default omit is suitable for low-cost batch scoring.",
...
def sidebar_config() -> dict:
    ...
    score_workers = st.sidebar.slider(t(lang, "score_workers"), 1, 64, 8, help=t(lang, "score_workers_help"))
    segment_input = st.sidebar.selectbox(t(lang, "segment_input"), ["rough", "full"], index=0, help=t(lang, "segment_input_help"))
    ...
    segment_thinking = st.sidebar.selectbox(t(lang, "segment_thinking"), ["omit", "enabled", "disabled"], index=0, help=t(lang, "segment_thinking_help"))
    score_thinking = st.sidebar.selectbox(t(lang, "score_thinking"), ["omit", "enabled", "disabled"], index=0, help=t(lang, "score_thinking_help"))
