...
- **重难点词汇表**: 每轮附带一份学生版词汇表（重难点阅读词汇 + 语法词汇变形两张表）。
- **合并多份词汇表**: 一个学期跑了十几轮就有十几份词汇表，一键合并成一张、去掉重复的词。
  在您自己的电脑上完成，几秒钟出结果（界面：基础模式 →「合并已有词汇表…」）。
...
# 把多份已生成的重难点词汇表合并成一份、去掉重复（本地运行，不消耗 API 额度）
python3 scripts/merge_vocab_docx.py \
...
- **完全本地免费运行**: 试卷本地切分、切分质量检查、答案修复、质量报告查看、最终 Word (.docx) 导出、
  **多份词汇表合并去重**、GUI 调试验收模式。
...
4. `高三英语精选试题_重难点词汇表_学生版.docx`：两张表（重难点阅读词汇 / 语法词汇变形），
   分页排好、可直接打印发给学生。多轮的词汇表可以用「合并已有词汇表…」合成一张。
